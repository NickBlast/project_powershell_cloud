...
- Current phase is raw-export-first: include `generated_at`, `tool_version`, and optional `dataset_version` metadata, but full schema validation is paused until data models stabilize.
...
- Maintain the wave/micro-PR cadence by updating `todo.md`, `audit_notes/`, and `CHANGELOG.md` with problem, solution, validation evidence, and follow-ups (note schema impact only when the future-phase work resumes).
