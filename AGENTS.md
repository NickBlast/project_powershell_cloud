...
- Deliver audit-grade evidence: deterministic exports with `generated_at`, `tool_version`, and optional `dataset_version`; never store secrets/PII in the repo. Schema validation is paused until datasets stabilize.
...
- `docs/` — reference docs, compliance mappings, prompt banks.
...
- Review relevant modules/scripts plus `docs/compliance` entries before editing.
...
- `pwsh -File ./scripts/export-<dataset>.ps1 -WhatIf` — dry-run dataset collection.
- Store sanitized sample outputs under `examples/` (or redacted `outputs/`), never committing secrets.

## 6. Planning & Micro-PR Workflow
1. **Plan** — produce a short checklist (touched files, test impact, validation steps) before coding; keep waves small.
...
- Preserve core metadata (`generated_at`, `tool_version`, optional `dataset_version`) on every dataset; reruns must be idempotent.
- Schema design/validation is paused until datasets stabilize; changes today focus on raw exports and logging.
...
- Capture project-specific behaviors (logging expectations, retries, future schema plans, tooling quirks) in `ai_project_rules.md`.
