...
- The current MVP focuses on **Azure** (Entra ID, Azure Resource Manager); **AWS** support exists where scripts are present. :contentReference[oaicite:1]{index=1}  
- Exports are written to `outputs/` and include a minimal header with:
  - `generated_at` (UTC ISO-8601 timestamp)
  - `tool_version` (tool/script version)
  - `dataset_version` (semantic dataset version; used for future-breaking changes) :contentReference[oaicite:2]{index=2}  

When proposing changes, keep the design centered on **deterministic, scriptable exports** that can be scheduled and audited in a financial-services environment.
...
- **Prerequisites / setup**
  - PowerShell 7.4+.
  - From the repo root:
    - Run `scripts/ensure-prereqs.ps1` to install required modules into the current user scope (Az, Microsoft.Graph, SecretManagement, PSScriptAnalyzer, Pester, etc.). :contentReference[oaicite:3]{index=3}  

- **Typical usage**
  - Azure export example:
    - `scripts/export-azure_scopes.ps1` (and similar) are the primary entry points for inventory. :contentReference[oaicite:4]{index=4}  
  - AWS export example:
    - `scripts/export-aws_*.ps1` where present.

- **Testing and validation**
  - Prefer to validate PowerShell changes with:
    - `Invoke-ScriptAnalyzer` over `.ps1`, `.psm1`, `.psd1` (no new errors; minimize warnings). :contentReference[oaicite:5]{index=5}  
...
Use **Microsoft Learn** guidance and **PSScriptAnalyzer** rules as your source of truth for PowerShell behavior and style, especially in code review. :contentReference[oaicite:6]{index=6}  

Key conventions:

- **Function and cmdlet style**
  - Prefer **advanced functions** with `[CmdletBinding()]` when adding reusable commands.
  - Use approved **Verb-Noun** naming where possible (see PowerShell cmdlet design guidelines). :contentReference[oaicite:7]{index=7}  
...
- **Comment-based help**
  - For any new public functions or scripts, include comment-based help using the standard syntax:
    - Either single-line `# .KEYWORD` style or `<# .KEYWORD … #>` blocks. :contentReference[oaicite:8]{index=8}  
...
- **Azure and Entra ID**
  - Use documented modules and patterns:
    ...
    - `Microsoft.Graph` / `Microsoft.Graph.Entra` for directory and identity operations. :contentReference[oaicite:9]{index=9}  
...
- **AI rules and agents**
  - `AGENTS.md`, `ai_project_rules.md`, and `docs/repo_design/repo_contract.md` are authoritative for project behavior and boundaries.
...
- If a PowerShell behavior or module API is unclear, assume **Microsoft Learn** is authoritative and prefer patterns from official examples over guesses. :contentReference[oaicite:10]{index=10}  
