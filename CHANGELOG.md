...
### Changed
- Completed WO-AI-001 by removing artificial intelligence and tooling references from scripts, modules, and operator-facing documentation, confirming via a repository-wide sweep that remaining mentions are isolated to agent-only guidance (for example, `AGENTS.md`, `ai_project_rules.md`, and `docs/reference/agent_onboarding.md`), and removing the work order entry from `.codex/work_orders.md`.
